# Baseline file from separated Major file based on columns given by UK Biobank parameters: 

baseline <- read.csv(file = 'C:/Users/User/Desktop/Data/Results/baseline_data.csv')

# Column name changes for baseline df: 

setnames(baseline,old=colnames(baseline), new = c('Eid',"Education","Diabetes","Age","Smoking","Drinking","ICD_code"))

# Baseline medication for df from baseline and medication.  
...
# Metformin medication rows taken from baseline and medication df merged file. 
...
# Non-metformin medicine extracted from df
...
# MAFLD without Metformin medicine from df
