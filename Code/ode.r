...
# 3.2 Define the function to calculate equilibrium
calculate_equilibrium <- function(parameters) {
  with(as.list(parameters), {
    V <- k1 * (C + T + E) * B / k2
    I <- k1 * (C + T + E) * B * alpha / k4
...
# Create a data frame with the equilibrium values
equilibrium_2 <- data.frame(V = equilibrium_values[1], I = equilibrium_values[2],L = equilibrium_values[3],
                             H = equilibrium_values[4],A = equilibrium_values[5])
print(equilibrium_2)


# 3.3 Define the ODE system with constant values :
ode_system1 <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {
    # Calculate derivatives
    dVdt <- k1 * (C + T + E) * B - k2 * V
    dIdt <- k2 * alpha * V - k4 * I
    ...
    dHdt <- k7 * A * (C + P) - k8 * H
    dAdt <- k9 - k7 * A * (C + P) + k8 * (1 - gamma) * H
    return(list(c(dVdt, dIdt, dLdt, dHdt, dAdt))) # Return the derivatives of state variables with respect to time
  })}

# Get parameter values from df_solution :
parameters <- c(k1 = df_solution$value[1], k2 = df_solution$value[2], k4 = df_solution$value[3], 
                k6 = df_solution$value[4], k7 = df_solution$value[5], k8 = df_solution$value[6], k9 = df_solution$value[7], 
                alpha = 0.6, beta = 0.5, gamma = 0.7)

# Initial conditions from equilibrium values
initial_state <- calculate_equilibrium(parameters) # Initial state of the system using the equilibrium values obtained earlier

# derivatives at initial condition
derivatives_start <- ode_system1(1, initial_state, parameters)

# Time points for evaluation
times <- seq(0, 0.99, length.out = 9) # Sequence of time points at which the ODE system is evaluated

# Solve the ODE system
solution3 <- data.frame(ode(y = initial_state, times = times, func = ode_system1, parms = parameters))
